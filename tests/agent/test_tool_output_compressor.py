...
def test_crusher_init_failed_sentinel():
    """Failed init sets sentinel so we don't retry forever."""
    call_count = 0
    ...
    with patch.object(comp, "_get_crusher", side_effect=fake_get):
        for _ in range(5):
            comp.compress_tool_result("x", "x" * 3000, config={"enabled": True})
        # _get_crusher was called once, sentinel prevents retries
    assert call_count == 5  # called once per tool call when always None
...
def test_md5_usedforsecurity_false():
    """MD5 hash uses usedforsecurity=False for FIPS compatibility."""
    import hashlib
    # Verify the module uses the correct call
    h = hashlib.md5(b"test", usedforsecurity=False).hexdigest()[:12]
    assert len(h) == 12
    assert h == hashlib.md5(b"test", usedforsecurity=False).hexdigest()[:12]
